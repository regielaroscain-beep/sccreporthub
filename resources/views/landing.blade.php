...
<head>
    ...
    <style>
        :root {
            --p:#4f46e5; --pd:#3730a3; --pl:#818cf8; --ac:#06b6d4;
            --ok:#10b981; --warn:#f59e0b; --err:#ef4444;
            --bg:#f1f5f9; --card:#fff; --border:#e2e8f0;
            --t1:#0f172a; --t2:#64748b; --t3:#94a3b8; --dark:#0f172a;
        }
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        html{scroll-behavior:smooth}
        body{font-family:'Inter',system-ui,sans-serif;background:var(--bg);color:var(--t1);font-size:.875rem;line-height:1.6;-webkit-font-smoothing:antialiased;overflow-x:hidden}
        a{text-decoration:none;color:var(--p)}

        /* NAV */
        .lnav{position:fixed;top:0;left:0;right:0;z-index:900;height:64px;background:var(--card);border-bottom:1px solid var(--border);box-shadow:0 1px 3px rgba(0,0,0,.05);display:flex;align-items:center}
        ...
        .lnav-brand-name{font-size:1rem;font-weight:800;letter-spacing:-.3px;background:linear-gradient(135deg,var(--p),var(--ac));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
        .lnav-brand-sub{font-size:.58rem;font-weight:600;color:var(--t3);text-transform:uppercase;letter-spacing:.8px;display:block}
        .lnav-links{display:flex;gap:28px}
        .lnav-links a{font-size:.845rem;font-weight:500;color:var(--t2);transition:color .15s}
        .lnav-links a:hover{color:var(--p)}
        .btn-si{border:1.5px solid var(--border);color:var(--t2);background:var(--card);border-radius:8px;padding:7px 18px;font-size:.845rem;font-weight:600;transition:all .15s;display:inline-flex;align-items:center}
        .btn-si:hover{border-color:var(--p);color:var(--p)}
        .btn-su{background:linear-gradient(135deg,var(--p),var(--ac));color:#fff;border:none;border-radius:8px;padding:7px 18px;font-size:.845rem;font-weight:600;box-shadow:0 2px 8px rgba(79,70,229,.3);transition:opacity .15s,transform .15s;display:inline-flex;align-items:center}
        ...
        .mm-cta .btn-su,.mm-cta .btn-si{width:100%;justify-content:center;padding:12px;font-size:.875rem;border-radius:10px}

        /* HERO */
        .hero{padding-top:64px;background:linear-gradient(155deg,#fff 0%,#f8faff 45%,#eef2ff 100%);border-bottom:1px solid var(--border);position:relative;overflow:hidden}
        .hero::before{content:'';position:absolute;inset:0;pointer-events:none;background:radial-gradient(ellipse 55% 50% at 85% 20%,rgba(79,70,229,.07) 0%,transparent 60%),radial-gradient(ellipse 35% 35% at 10% 85%,rgba(6,182,212,.05) 0%,transparent 55%)}
        .hero-body{padding:52px 0 0;position:relative;z-index:1}

        /* Hero eyebrow — small overline style */
        .hero-eyebrow{display:flex;align-items:center;gap:10px;margin-bottom:18px}
        .hero-eyebrow-line{height:2px;width:28px;background:linear-gradient(90deg,var(--p),var(--ac));border-radius:2px;flex-shrink:0}
        .hero-eyebrow-text{font-size:.7rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--t2)}

        .hero-h{font-size:clamp(2rem,4vw,3rem);font-weight:800;color:var(--t1);line-height:1.1;letter-spacing:-.5px;margin-bottom:16px}
        .hero-h span{background:linear-gradient(135deg,var(--p),var(--ac));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
        .hero-p{font-size:.975rem;color:var(--t2);line-height:1.75;margin-bottom:28px;max-width:420px}
        .btn-hero{display:inline-flex;align-items:center;gap:8px;background:linear-gradient(135deg,var(--p),var(--ac));color:#fff;border:none;border-radius:8px;padding:12px 26px;font-size:.875rem;font-weight:600;box-shadow:0 2px 12px rgba(79,70,229,.3);transition:opacity .15s,transform .15s,box-shadow .15s}
        .btn-hero:hover{opacity:.9;transform:translateY(-1px);box-shadow:0 6px 20px rgba(79,70,229,.4);color:#fff}
        .btn-hero2{display:inline-flex;align-items:center;gap:8px;background:var(--card);color:var(--t2);border:1.5px solid var(--border);border-radius:8px;padding:12px 22px;font-size:.875rem;font-weight:500;transition:border-color .15s,color .15s}
        .btn-hero2:hover{border-color:var(--p);color:var(--p)}

        /* Dashboard preview */
        .preview-wrap{position:relative;padding:0 0 0 20px}
        .preview-wrap::before{content:'';position:absolute;top:16px;left:0;right:-16px;bottom:-16px;background:linear-gradient(135deg,#eef2ff,#e0f2fe);border-radius:14px;z-index:0}
        .preview-card{background:var(--card);border:1px solid var(--border);border-radius:12px;box-shadow:0 8px 32px rgba(79,70,229,.12),0 2px 8px rgba(0,0,0,.06);overflow:hidden;margin-top:36px;position:relative;z-index:1}
        .preview-topbar{background:var(--bg);border-bottom:1px solid var(--border);padding:10px 16px;display:flex;align-items:center;gap:6px}
        .ptb-dot{width:10px;height:10px;border-radius:50%}
        .ptb-url{flex:1;background:var(--card);border:1px solid var(--border);border-radius:4px;padding:3px 10px;font-size:.65rem;color:var(--t3);margin:0 8px}
        .preview-body{padding:16px}
        .preview-header{font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--t3);margin-bottom:10px}
        .pstat-row{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:12px}
        ...
        .pstat-l{font-size:.6rem;color:var(--t3);text-transform:uppercase;letter-spacing:.05em;margin-top:3px;font-weight:500}
        .ptable{width:100%;border-collapse:collapse;font-size:.72rem}
        .ptable th{text-align:left;padding:6px 8px;color:var(--t3);font-weight:600;text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid var(--border);font-size:.65rem}
        .ptable td{padding:8px;color:var(--t2);border-bottom:1px solid #f1f5f9}
        .ptable tr:last-child td{border-bottom:none}
        .pbadge{font-size:.6rem;font-weight:600;padding:2px 7px;border-radius:20px}

        /* SECTIONS */
        .sec{padding:80px 0}
        .sec-w{background:var(--card);border-top:1px solid var(--border);border-bottom:1px solid var(--border)}
        .sec-label{font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:var(--p);margin-bottom:8px}
        .sec-h{font-size:clamp(1.5rem,2.8vw,2rem);font-weight:800;color:var(--t1);line-height:1.2;letter-spacing:-.3px}
        .sec-p{font-size:.9rem;color:var(--t2);max-width:500px;margin:0 auto;line-height:1.7}

        /* Feature cards */
        .fc{background:var(--card);border:1px solid var(--border);border-left:3px solid transparent;border-radius:14px;padding:24px;height:100%;box-shadow:0 1px 3px rgba(0,0,0,.04);transition:transform .2s,box-shadow .2s,border-color .2s}
        ...
        .fc-h{font-size:.875rem;font-weight:700;color:var(--t1);margin-bottom:6px}
        .fc-p{font-size:.82rem;color:var(--t2);line-height:1.65}

        /* How it works */
        .how-card{background:var(--card);border:1px solid var(--border);border-radius:12px;padding:22px;display:flex;gap:14px;box-shadow:0 1px 3px rgba(0,0,0,.04);transition:border-color .2s;position:relative}
        .how-card:hover{border-color:#c7d2fe}
        .how-arrow{display:none;position:absolute;right:-18px;top:50%;transform:translateY(-50%);z-index:2;color:var(--t3);font-size:.75rem}
        @media(min-width:768px){.col-md-6:nth-child(odd) .how-arrow{display:block}}
        .how-n{width:32px;height:32px;border-radius:8px;flex-shrink:0;background:linear-gradient(135deg,var(--p),var(--ac));color:#fff;font-size:.8rem;font-weight:700;display:flex;align-items:center;justify-content:center}
        .how-h{font-size:.845rem;font-weight:700;color:var(--t1);margin-bottom:4px}
        .how-p{font-size:.78rem;color:var(--t2);line-height:1.6}

        /* Role cards */
        .rocard{background:var(--card);border:1px solid var(--border);border-radius:14px;padding:28px 22px;height:100%;box-shadow:0 1px 3px rgba(0,0,0,.04);transition:transform .2s,box-shadow .2s,border-color .2s}
        ...
        .ro-h{font-size:.9rem;font-weight:700;color:var(--t1);margin-bottom:6px}
        .ro-p{font-size:.82rem;color:var(--t2);line-height:1.65;margin-bottom:14px}
        .ro-tag{font-size:.65rem;font-weight:600;padding:2px 8px;border-radius:20px;display:inline-block;margin:2px}

        /* CTA */
        .cta-box{background:linear-gradient(135deg,var(--dark) 0%,#1e1b4b 50%,#312e81 100%);border-radius:16px;padding:60px 40px;text-align:center;position:relative;overflow:hidden}
        ...
        /* Footer */
        footer{background:var(--dark);color:rgba(255,255,255,.4);padding:44px 0 24px;font-size:.82rem;position:relative}
        footer::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,var(--p),var(--ac))}
        .ft-name{font-size:.9rem;font-weight:800;color:#fff}
        ...
        .ft-col-h{font-size:.6rem;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:rgba(255,255,255,.2);margin-bottom:12px}
        ...
        @media(max-width:767px){
            ...
            .hero-p{font-size:.875rem;margin-bottom:22px}
            .preview-card{margin-top:24px}
            .pstat-row{grid-template-columns:repeat(2,1fr)}
            .sec{padding:56px 0}
            .sec-h{font-size:1.4rem}
            .fc{padding:20px}
            .fc-icon{width:38px;height:38px;font-size:.95rem;margin-bottom:10px}
    ...
    <div class="container">
        <div class="hero-body">
            <div class="row align-items-center g-5">
                <div class="col-lg-5">
                    <!-- Eyebrow label — line + text, not a button -->
                    <div class="hero-eyebrow">
                        <div class="hero-eyebrow-line"></div>
                        <span class="hero-eyebrow-text">Campus Facility Management System</span>
                    </div>
                    ...
                    <div class="preview-wrap">
                        <div class="preview-card">
                            ...
                            <div class="preview-body">
                                ...
                                <div class="pstat-row">
                                    <div class="pstat"><div class="pstat-n" style="color:var(--p);">24</div><div class="pstat-l">Total</div></div>
                                    <div class="pstat"><div class="pstat-n" style="color:var(--warn);">8</div><div class="pstat-l">Pending</div></div>
                                    <div class="pstat"><div class="pstat-n" style="color:var(--ac);">10</div><div class="pstat-l">Ongoing</div></div>
                                    <div class="pstat"><div class="pstat-n" style="color:var(--ok);">12</div><div class="pstat-l">Resolved</div></div>
                                </div>
                                <table class="ptable">
        ...
        <div class="row g-3 fu">
            ...
            <div class="col-md-6">
                <div class="how-card">
                    ...
                    <div><div class="how-h">Admin Reviews & Assigns</div><div class="how-p">Admin approves and assigns it to the right maintenance staff.</div></div>
                </div>
            </div>
            <div class="col-md-6" style="position:relative;">
                <div class="how-card">
            ...
            <div class="col-md-6">
                <div class="how-card">
                    ...
                    <div><div class="how-h">Verified & Rated</div><div class="how-p">Admin verifies completion. Faculty rates the service. Ticket closed.</div></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="container">
        <div class="cta-box fu">
            <div style="position:relative;z-index:1;">
                ...
                <a href="{{ route('register') }}" style="display:inline-flex;align-items:center;gap:8px;background:#fff;color:var(--p);border:none;border-radius:8px;padding:12px 28px;font-size:.9rem;font-weight:700;box-shadow:0 2px 12px rgba(0,0,0,.15);transition:transform .15s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform=''">
    ...
    <div class="container">
        <div class="row g-4">
            <div class="col-md-5">
                ...
                <p style="font-size:.78rem;line-height:1.6;max-width:260px;">Campus Facility Status Report &amp; Monitoring System — Southern Christian College, Midsayap, Cotabato.</p>
            </div>
